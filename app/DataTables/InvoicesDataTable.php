...
class InvoicesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('status', function ($row) {
            ...
            if ($row->credit_note) {
                $status .= '<span class="span-yellow">' . __('app.credit-note').'</span>' ;
            }
            else {
                if ($row->status == 'unpaid') {
                    $status .= '<span class="span-red">' . __('app.' . $row->status) .'</span>';
                }
                elseif ($row->status == 'paid') {
                    $status .= '<span class="span-green">' . __('app.' . $row->status) .'</span>';
                }
                elseif ($row->status == 'draft') {
                    $status .= '<span class="span-blue">' . __('app.' . $row->status) .'</span>';
                }
                elseif ($row->status == 'canceled') {
                    $status .= '<span class="span-red">' . __('app.' . $row->status) .'</span>';;
                }
                else {
                    $status .= '<span class="span-green">' . __('modules.invoices.partial'). '</span>';
                }
            }
